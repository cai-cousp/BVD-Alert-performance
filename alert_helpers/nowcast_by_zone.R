...
#' @param min_date Date. Earliest onset date to retain; `NULL` keeps all.
#' @param recent_window_days Integer. Number of days in the recent window.
#' @param min_recent_cases Integer. Minimum cases in recent window required for
#'   EpiNow2 fitting (otherwise empirical fallback is used).
#' @param min_series_days Integer. Minimum days in series required for EpiNow2.
#' @param delay_min_onset Date. Optional minimum onset date for delay estimation.
#' @param generation_time An EpiNow2 distribution spec for the generation time.
#' @param dist_samples Integer. Total posterior draws requested from the
#'   bootstrapped truncation-distribution fit.
#' @param stan Optional EpiNow2 stan_opts.
#' @param seed Integer. Optional RNG seed for reproducibility.
#' @param fit_fun Function. Nowcast fitting function; defaults to
#'   `fit_nowcast_epinow2`.
#' @param cache_path Character. Optional path to save or load cached nowcasts.
#' @param snapshot_key Character. Optional snapshot identifier for caching.
#' @param verbose Logical. Passed to fitting function.
#'
