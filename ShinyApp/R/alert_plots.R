...
extract_latest_evd_date <- function(
    base_path = if (exists("evd17_root", inherits = TRUE)) file.path(evd17_root, "DataCleaning", "data", "Output") else ""
) {
...
plot_adequacy_stacked <- function(
  ...
  if (is.numeric(date_breaks)) {
    if (length(date_breaks) != 1L || !is.finite(date_breaks) ||
        date_breaks <= 0 || date_breaks %% 1L != 0) {
      rlang::abort("`date_breaks` must be a single positive whole number of weeks.")
    }
    date_breaks <- paste(date_breaks, "weeks")
  } else if (!is.null(date_breaks) &&
             (!is.character(date_breaks) || length(date_breaks) != 1L)) {
    rlang::abort("`date_breaks` must be `NULL`, a single positive whole number of weeks, or a single character string.")
  }
