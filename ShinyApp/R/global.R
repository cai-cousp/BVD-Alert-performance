...
find_evd17_root <- function(root_dir) {
  candidates <- c(
    Sys.getenv("EVD17_ROOT", ""),
    dirname(root_dir),
    dirname(dirname(root_dir))
  )
  for (cand in candidates) {
    if (nzchar(cand) && dir.exists(file.path(cand, "DataCleaning"))) {
...
evd17_root <- find_evd17_root(root_dir)
output_base <- file.path(root_dir, "output")
data_folder <- file.path(evd17_root, "DataCleaning", "data", "Output")
# Maps are at EVD17/Maps/
maps_base_dir <- file.path(evd17_root, "Maps")
map_dir <- file.path(maps_base_dir, "health_zones")
...
if (file.exists(file.path(app_dir, "R", "synthesis_helpers.R"))) {
  source(file.path(app_dir, "R", "synthesis_helpers.R"))
}
if (file.exists(file.path(app_dir, "R", "map_data_helpers.R"))) {
  source(file.path(app_dir, "R", "map_data_helpers.R"))
}
if (file.exists(file.path(app_dir, "R", "alert_plots.R"))) {
  source(file.path(app_dir, "R", "alert_plots.R"))
} else if (file.exists(file.path(root_dir, "R", "alert_plots.R"))) {
...
sync_alert_data <- function(output_base, root_dir, data_folder, force_rerun = FALSE) {
  # Skip sync if running in WebAssembly or if external pipeline scripts/data don't exist
  if (isTRUE(getOption("webr.app", FALSE)) || nzchar(Sys.getenv("WEBR"))) {
    return(invisible(FALSE))
  }
  script_01 <- file.path(root_dir, "R", "01_alert_thresholds.R")
  if (!file.exists(script_01) || !dir.exists(data_folder)) {
    return(invisible(FALSE))
  }
...
# --- Load data from bundled directory (for WebAssembly / GitHub Pages) --------
load_bundled_data <- function(bundled_dir) {
  message("Loading data from bundled directory: ", bundled_dir)
  synth_path <- if (file.exists(file.path(bundled_dir, "01_thresholds_synthesis.rds"))) {
    file.path(bundled_dir, "01_thresholds_synthesis.rds")
  } else {
    file.path(bundled_dir, "synthesis.rds")
  }
  synthesis <- readRDS(synth_path) |> tibble::as_tibble()
  if ("week_start" %in% names(synthesis)) {
    synthesis <- synthesis |> mutate(week_start = as.Date(.data$week_start))
  }
  synthesis <- synthesis |>
    mutate(across(where(is.numeric), \(x) if (all(is.na(x))) x else round(x, 3)))

  param_path <- if (file.exists(file.path(bundled_dir, "01_intermediate_parameters.rds"))) {
    file.path(bundled_dir, "01_intermediate_parameters.rds")
  } else if (file.exists(file.path(bundled_dir, "intermediate_params.rds"))) {
    file.path(bundled_dir, "intermediate_params.rds")
  } else NULL

  intermediate_params <- if (!is.null(param_path) && file.exists(param_path)) {
    readRDS(param_path)
  } else list()

  trend_path <- if (file.exists(file.path(bundled_dir, "02_trends_smooth_adeq.rds"))) {
    file.path(bundled_dir, "02_trends_smooth_adeq.rds")
  } else {
    file.path(bundled_dir, "trends_smooth_adeq.rds")
  }
  trends_smooth <- readRDS(trend_path) |> tibble::as_tibble()
  if ("week_start" %in% names(trends_smooth)) {
    trends_smooth <- trends_smooth |> mutate(week_start = as.Date(.data$week_start))
  } else if ("threshold_time_key" %in% names(trends_smooth)) {
    trends_smooth <- trends_smooth |> mutate(week_start = as.Date(.data$threshold_time_key))
  }
  trends_smooth_adeq <- trends_smooth
...
# --- Determine data loading mode (bundled vs live output) ---------------------
is_webr <- isTRUE(getOption("webr.app", FALSE)) || nzchar(Sys.getenv("WEBR"))
bundled_data_dir <- file.path(app_dir, "data")
has_bundled_data <- dir.exists(bundled_data_dir) &&
  (file.exists(file.path(bundled_data_dir, "01_thresholds_synthesis.rds")) ||
   file.exists(file.path(bundled_data_dir, "synthesis.rds"))) &&
  (file.exists(file.path(bundled_data_dir, "02_trends_smooth_adeq.rds")) ||
   file.exists(file.path(bundled_data_dir, "trends_smooth_adeq.rds")))

use_bundled_data <- is_webr ||
  isTRUE(getOption("bvd.use_bundled_data", FALSE)) ||
  !dir.exists(output_base) ||
  (has_bundled_data && !file.exists(file.path(root_dir, "R", "01_alert_thresholds.R")))
...
if (!use_bundled_data) {
  ...
  # --- Load from bundled data in ShinyApp/data ---
  data <- load_bundled_data(bundled_data_dir)
}
...
all_hz_individual <- sort(setdiff(
  unique(trends_smooth_adeq$zone_sante_notification),
  notification_ensemble_label
))

# --- Prepare the interactive notification performance map --------------------
notification_map_data <- if (file.exists(file.path(bundled_data_dir, "notification_map_data.rds"))) {
  readRDS(file.path(bundled_data_dir, "notification_map_data.rds"))
} else {
  tryCatch({
    geography <- read_alert_grid3_geography(health_zone_dir = map_dir)
    prepare_notification_map_data(
      recent_adequacy = recent_adequacy,
      trends = trends_smooth_adeq,
      geography = geography
    )
  }, error = function(e) {
    warning(
      "Interactive notification map could not be prepared: ",
      conditionMessage(e),
      call. = FALSE
    )
    NULL
...
if (!is.null(notification_map_data) && inherits(notification_map_data, "sf") && !"map_id" %in% names(notification_map_data)) {
  notification_map_data$map_id <- sprintf("notification_hz_%04d", seq_len(nrow(notification_map_data)))
}
notification_map_lookup_table <- notification_map_lookup(notification_map_data)
...
province_label_data <- if (file.exists(file.path(bundled_data_dir, "province_label_data.rds"))) {
  readRDS(file.path(bundled_data_dir, "province_label_data.rds"))
} else if (!is.null(province_map_data)) {
  create_province_label_points(province_map_data)
} else {
  tibble::tibble(
