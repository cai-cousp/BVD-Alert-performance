...
# Ensure Alert_performance_report.html is synchronized into ShinyApp/ before Shinylive export
...
report_dest_app <- file.path(app_dir, "Alert_performance_report.html")
if (file.exists(report_src)) {
  if (!file.exists(report_dest_app) || file.info(report_src)$mtime > file.info(report_dest_app)$mtime) {
    ...
    message("Synchronized Alert_performance_report.html into ShinyApp/ (", round(file.size(report_src) / (1024 * 1024), 2), " MB).")
  }
}
...
tryCatch({
  ...
  # Copy static report to export directory for direct static access as well
  if (file.exists(report_dest_app)) {
    file.copy(report_dest_app, file.path(dest_path, "Alert_performance_report.html"), overwrite = TRUE)
